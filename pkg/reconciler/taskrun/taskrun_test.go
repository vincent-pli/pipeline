	...
	for _, tc := range []struct {
		...
		wantPod: tb.Pod("test-taskrun-substitution-pod-123456", "foo",
			...
			tb.PodSpec(
				...
				getPlaceToolsInitContainer(),
				getMkdirResourceContainer("myimage", "/workspace/output/myimage", "mz4c7"),
				...
				tb.PodContainer("step-image-digest-exporter-image-resource-mssqb", "override-with-imagedigest-exporter-image:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/tools/3", "-post_file", "/builder/tools/4", "-entrypoint", "/ko-app/imagedigestexporter", "--",
						"-images", "[{\"name\":\"image-resource\",\"type\":\"image\",\"url\":\"gcr.io/kristoff/sven\",\"digest\":\"\",\"OutputImageDir\":\"/workspace/output/myimage\",\"digestImage\":\"override-with-imagedigest-exporter-image:latest\"}]"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
					tb.VolumeMount("tools", "/builder/tools"),
					tb.VolumeMount("workspace", workspaceDir),
